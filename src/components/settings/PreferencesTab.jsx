...
const PreferencesTab = ({ preferences, updatePreferences }) => {
  const handleThemeChange = (theme) => {
    ...
    if (typeof window !== 'undefined') {
      try {
        window.localStorage.setItem('starscale.theme', theme);
        window.dispatchEvent(new CustomEvent('starscale-theme-change', { detail: theme }));
      } catch (e) {
        // fail silently; theme will still apply on next reload
      }
    }
  };
  ...
  const themes = [
    ...
    { id: 'focus', label: 'Focus Dark', icon: Moon },
  ];
              ...
              return (
                <button
                  ...
                  className={`p-6 rounded-xl border-2 transition-all flex flex-col items-center gap-3 ${
                    active
                      ? 'border-cyan-500 bg-cyan-500/10 shadow-[0_0_25px_rgba(34,211,238,0.3)]'
                      : 'border-white/5 bg-black/20 hover:border-white/20 hover:bg-white/5'
                  }`}
                >
                  <div className={`p-3 rounded-full ${active ? 'bg-cyan-500 text-white' : 'bg-white/10 text-gray-400'}`}>
